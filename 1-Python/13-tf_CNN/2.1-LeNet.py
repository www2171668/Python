...
x = tf.placeholder(tf.float32, shape=[None, input_dim], name='x')   # 图像像素数据
y = tf.placeholder(tf.float32, shape=[None, n_classes], name='y')   # 图像类型
learn_rate = tf.placeholder(tf.float32, name='learn_rate')   # 控制学习率减幅

def learn_rate_func(epoch):   # 根据给定的迭代批次，更新产生一个学习率的值
    return learn_rate_base * (0.9 ** int(epoch / 10))   # 每10次迭代，学习率减小一次
...
'''
    ...
    ①、卷积：
        ...
        input：4维格式的输入数据，具体格式和data_format有关；
            当data_format为NHWC时，input的格式为: [batch_size, height, weight, channels] => [批次中的图片数目，图片的高度，图片的宽度，图片的通道数]
            当data_format为NCHW时，input的格式为: [batch_size, channels, height, weight] => [批次中的图片数目，图片的通道数，图片的高度，图片的宽度]
    ...
    ③、池化
        -》tf.nn.max_pool(value, ksize, strides, padding, data_format="NHWC", name=None)：和conv2一样，需要给定窗口大小和步长，
            ...
            value：输入的数据，[batch_size, height, weight, channels]格式
...
'''
def Le_net(x):   #  注意variable_scope和get_variable的配合★★★
    # 0. 输入层（黑白图片）
    with tf.variable_scope('input0'):
        # 将输入的x的格式转换为规定的格式:[None, input_dim] -> [Batch, height, weight, channels]
        net = tf.reshape(x, shape=[-1, 28, 28, 1])   # Batch是图片的批量数量，用-1填充  (?, 28, 28, 1)
    # 1. 卷积层
    with tf.variable_scope('conv1'):
        net = tf.nn.conv2d(input=net, filter=tf.get_variable('w', [5, 5, 1, 20]), strides=[1, 1, 1, 1], padding='SAME')   # (?, 28, 28, 20)   VALID(?, 24, 24, 20)
    ...
    with tf.variable_scope('fc5'):
        ...
        net = tf.add(tf.matmul(net, tf.get_variable('w', [7 * 7 * 50, 500])), tf.get_variable('b', [500]))   # X*W+b  形成500个神经元  [7 * 7 * 50, 500]->(?, 500)
        net = tf.nn.relu(net)
    # 6. 全连接
    with tf.variable_scope('fc6'):
        net = tf.add(tf.matmul(net, tf.get_variable('w', [500, n_classes])), tf.get_variable('b', [n_classes]))   # (?, 500)->(?, 10)
...
with tf.Session() as sess:
    ...
    while True:
        avg_loss = 0
        total_batch = int(train_sample_number / batch_size)   # 计算出总的批次
        batch_xs = []
        batch_ys = []
