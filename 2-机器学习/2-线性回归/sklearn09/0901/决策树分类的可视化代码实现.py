...
algo = DecisionTreeClassifier(criterion='gini', max_depth=5, min_samples_split=10, random_state=0)
...
# 可视化方式一：将决策树的内部结构输出为dot的格式文件，然后使用graphviz对dot文件进行转换操作
# dot命令：dot -Tpdf iris.dot -o iris.pdf 或者dot -Tpng iris.dot -o iris.png
from sklearn import tree

with open('iris.dot', 'w') as writer:
    tree.export_graphviz(decision_tree=algo, out_file=writer)

# 可视化方式二：直接使用pydotpuls python库将数据输出为pdf或者png的格式
from sklearn import tree
import pydotplus

dot_data = tree.export_graphviz(decision_tree=algo, out_file=None,
                                feature_names=['A', 'B', 'C', 'D'],
                                # class_names=Y_label_values,
                                class_names=['1', '2', '3'],
