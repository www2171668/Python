...
def fit(X, Y, alpha=0.0001, max_iter=None, tol=1e-5, fit_intercept=True):
    ...
    if max_iter is None:
        max_iter = 20000
    max_iter = max_iter if max_iter > 0 else 20000
    ...
    for i in range(max_iter):
        ...
        # d. 计算在当前模型参数的情况下，损失函数（sum_j）的值。损失函数是相对于回归算法的最小二乘法说明的
        sum_j = 0.0
        for k in range(m):
            y_true = Y[k][0]
            ...
            try:
                sum_j += math.pow(y_true - y_predict, 2)   # 目标函数θ标准求解公式 ；当数据过大会产生越界现象，此时加大迭代次数，降低alpha值
            except:
                print("ERROR")
        sum_j /= m
...
if __name__ == '__main__':
    ...
    x.shape = -1, 1
    y.shape = -1, 1
    poly = PolynomialFeatures(degree=2,include_bias=False)   # 多项式拓展
    x = poly.fit_transform(x)   # ，并训练 归一
    ...
    # b. 使用自定义的梯度下降实现代码
    theta, intercept, sum_j = fit(x, y, alpha=0.001, max_iter=20000)
    ...
    x_hat.shape = -1, 1
    x_hat = poly.transform(x_hat)   # 归一化测试集

    y_hat = model.predict(x_hat)   # 预测
    y_hat2 = predict_X(x_hat, theta, intercept)   # 自定义
