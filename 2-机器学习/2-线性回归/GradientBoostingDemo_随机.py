...
def fit(X, Y, alpha=0.0001, max_iter=None, tol=1e-5, fit_intercept=True):
    ...
    for i in range(max_iter):
        ...
        if np.abs(previous_j - sum_j) < tol:
            break
    print("迭代{}次后，损失函数值为:{}".format(i, previous_j))
...
if __name__ == '__main__':
    ...
    # b. 使用自定义的梯度下降实现代码
    theta, intercept, sum_j = fit(x, y, alpha=0.0001, max_iter=20000)
    s2 = r2_score(y, predict_X(x, theta, intercept))   # —— r2_score(y_true,y_pred)
    ...
    y_test2 = predict_X(x_test, theta, intercept)

    # 画图看一下
    plt.plot(x, y, 'ro', ms=10)
    plt.plot(x_test, y_test, color='#b624db', lw=2, label=u'sklearn线性模型，$R^2$:%.3f' % s1)
    plt.plot(x_test, y_test2, color='#0049b6', lw=2, label=u'自定义线性模型，$R^2$:%.3f' % s2)
