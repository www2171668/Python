...
if "../" not in sys.path:
    sys.path.append("../")
from lib.envs.MCcontrol import GridWorldEnv
...
def on_policy_MC_control(env, num_episodes, discount_factor=1.0,
                         epsilon=0.1):  # Monte Carlo Control using Epsilon-Greedy policies.
    """
    ...
    for i_episode in range(1, num_episodes + 1):  # 开始迭代
        ...
        for t in range(100):  # 对本次episode进行采样。实际采样不到这么多次，很快就停了
            ...
            probs = policy(state)  # 通过ε-贪婪策略获得动作概率 —— action_probability [0.925 0.025 0.025 0.025]
            action = np.random.choice(np.arange(len(probs)), p=probs)  # 保证了探索性。返回的是动作编号，0或1
            if env.isOutTheScoup(action):
...
print()
print("状态 ,[向上的Q值  ,向下的Q值  ,向左的Q值  ,向右的Q值  ]")
for k in sorted(Q):
    print(k,Q[k])
