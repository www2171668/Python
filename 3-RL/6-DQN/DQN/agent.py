...
class DQN(object):
    ...
    def construct_model(self, gpu):
        ...
        with tf.device(device):
            ...
            with tf.name_scope('q_network'):
                self.output_Q = self.network(self.input_state)   # 预测网络输出预测Q值向量，每一个元素对应输入的动作元素

            with tf.name_scope('optimize'):
                self.input_action = tf.placeholder(tf.float32, [None, self.action_dim])
                self.target_Q = tf.placeholder(tf.float32, [None])
                # Q value of the selceted action 预测Q值
                action_Q = tf.reduce_sum(tf.multiply(
                    self.output_Q, self.input_action), reduction_indices=1)   # 将预测Q值向量元素与对应输入的动作点乘，得到预测Q值

                self.loss = tf.reduce_mean(tf.square(self.target_Q - action_Q))   # 损失函数
    ...
    def update_model(self):
        ...
        })   # 从这里也可以看出，虽然是batch方法，但是输入的时候是可以忽视这个batch的运算规则，直接把batchu数据输入仅网络
